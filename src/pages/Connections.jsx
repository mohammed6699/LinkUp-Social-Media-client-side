...
import { dummyConnectionsData as connections, dummyFollowersData as follwers, 
  dummyFollowingData as following, dummyPendingConnectionsData as pendingconnections
 } from '../assets/assets';
function Connections() {
  const navigate = useNavigate()
  const dataArray =  [
    {label: 'Follwers', value: follwers, icon: Users},
    ...
    <div className='min-h-screen bg-slate-100'>
      <div className='max-w-6xl mx-auto p-6'>
        ...
        <div className='flex  gap-6 mt-5'>
          {(() => {
            const tabData = dataArray.find((item) => item.label === currentTab);
            if (!tabData) return null;
            return tabData.value.map((user) => (
              <div key={user._id}
